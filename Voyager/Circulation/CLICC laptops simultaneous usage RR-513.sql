...
with items as (
  ...
  where 
  ( it.item_type_code = 'cclaptop'
    and   ib.item_barcode like 'CBK%' or ib.item_barcode like 'HP%' or ib.item_barcode like 'MP%'
  )
  or    ( it.item_type_code = 'ccipad')
...
, sample_times as (
  ...
  from (
    select 
      ...
      to_date('20191125', 'YYYYMMDD') as start_date
    , to_date('20191126', 'YYYYMMDD') as end_date
    from dual
  )
...
, transactions as (
  ...
  where cta.charge_date between to_date('20191125', 'YYYYMMDD') and to_date('20191126', 'YYYYMMDD')
