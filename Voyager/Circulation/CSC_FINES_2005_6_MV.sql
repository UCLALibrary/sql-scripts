...
REM VGER_REPORT CSC_FINES_2005_6_MV

  CREATE MATERIALIZED VIEW "VGER_REPORT"."CSC_FINES_2005_6_MV"
...
decode(SUM(decode(ff.fine_fee_type,   1,   decode(fft.trans_type,   1,   1,   0),   0)),   0,   decode(SUM(decode(ff.fine_fee_type,   1,   decode(fft.trans_type,   2,   1,   0),   0)),   0,   0,   1),   1) AS
overdue,
  SUM(decode(ff.fine_fee_type,   1,   decode(fft.trans_type,   1,   1,   0),   0)) AS
overdue_payment,
  SUM(decode(ff.fine_fee_type,   1,   decode(fft.trans_type,   2,   1,   0),   0)) AS
overdue_forgive,
  decode(SUM(decode(ff.fine_fee_type,   2,   decode(fft.trans_type,   1,   1,   0),   0)),   0,   decode(SUM(decode(ff.fine_fee_type,   2,   decode(fft.trans_type,   2,   1,   0),   0)),   0,   0,   1),   1) AS
lost_item_replacement,
  SUM(decode(ff.fine_fee_type,   2,   decode(fft.trans_type,   1,   1,   0),   0)) AS
lost_item_replacement_payment,
  SUM(decode(ff.fine_fee_type,   2,   decode(fft.trans_type,   2,   1,   0),   0)) AS
lost_item_replacement_forgive
...
REM VGER_REPORT CSC_FINES_2005_6_MV
   ...
   COMMENT ON TABLE "VGER_REPORT"."CSC_FINES_2005_6_MV"  IS 'snapshot table for snapshot VGER_REPORT.CSC_FINES_2005_6_MV';
