/*	Counts of audio discs (analog), as defined for annual ARL/UCOP statistics
...
, unit_items as (
  ...
  where item_type_name = 'Sound Disk'
  and	(	hol_852h like 'LP%'
    or	(	SubStr(hol_007, 1, 1) = 's'
      and	SubStr(hol_007, 4, 1) in ('a', 'b', 'c', 'd', 'e')
		)
	)
...
, srlf_items as (
  select *
  from vger_report.arl_stats
  where item_type_name = 'SRLF Nonprint'
