/*	Counts of microfilm reels, as defined for annual ARL/UCOP statistics
...
, unit_items as (
  ...
  where item_type_name in ('Microfilm Book', 'Microfilm Journal')
  and mfhd_loc not like 'sr%'
)
, srlf_items as (
  select *
  from vger_report.arl_stats
  where item_type_name in ('Microfilm Book', 'Microfilm Journal', 'SRLF Nonprint')
  and mfhd_loc like 'sr%'
  and	(	Upper(hol_852k) like '%MICROFILM%'
		or	Upper(hol_852k) like '%MICRO FILM%'
    or	Upper(hol_852k) like '%MICRO-FILM%'
  )
)
select
